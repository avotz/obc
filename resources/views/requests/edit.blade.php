...
<div id="infoBox" class="alert alert-success" ></div>
 ...
 <div class="content bg-image" style="background-image: url('/img/photo-profile.jpg');">
    <div class="push-50-t push-15 clearfix">
        ...
        <h1 class="h2 text-white push-5-t animated zoomIn" title="Editar solicitude de cotización">Edit Quotation Request</h1>
    ...
    <div class="row">
        ...
        <div class="block">
                ...
                <div class="block-content">
                    <form class="js-validation-register form-horizontal push-50" method="POST" action="/requests/{{ $quotationRequest->id }}" enctype="multipart/form-data">
                        <input type="hidden" name="_method" value="PUT">              
                        {{ csrf_field() }}
                        @if(!$quotationRequest->quotations->count()) 
                            @include('requests/partials/form')
                        @else
                            @include('requests/partials/show') 
                        @endif
